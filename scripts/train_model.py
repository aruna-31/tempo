#!/usr/bin/env python3
"""
TEMPO — Rigorous Spatial-Temporal Behavior Training & Generalization Benchmark
=============================================================================
1. Fine-tunes ResNet-18 spatial backbone (layer4 + classifier) on classroom crops.
2. Extracts 512-dim sequence embeddings for 100% behavior-pure 16-frame continuous sequences.
3. Trains and benchmarks ResNet-18 + RNN, LSTM, and GRU on the 65-video train split.
4. Uses the 14-video validation split strictly for model selection (best validation macro-F1).
5. Evaluates the selected champion model on the completely untouched 14-video test split:
     - Overall Accuracy, Macro-F1, Macro-Precision, Macro-Recall
     - Per-class Precision, Recall, F1, and Support
     - Confusion Matrix
     - Per-video Accuracy, Macro-F1, and predictions for each of the 14 unseen test videos
     - Prediction Class Distribution
6. Acceptance Gate:
     - If Test Accuracy > 85% AND Test Macro-F1 > 85%: updates production checkpoint.
     - Else: preserves production checkpoint and metadata untouched.
"""
import csv
import json
import math
...
# ── Hyperparameters ──
SPATIAL_SAMPLES_PER_CLASS = 1500  # Expanded balanced crops per class for layer4 fine-tuning
SPATIAL_EPOCHS = 5
SPATIAL_BS = 32
SPATIAL_LR_BACKBONE = 5e-5        # Smaller LR for CNN backbone layer4
...
train_transform = T.Compose([
    ...
    T.RandomAffine(degrees=6, translate=(0.04, 0.04), scale=(0.95, 1.05)),
    T.ToTensor(),
...
def report_pre_training_verification(rows: List[Dict], seq_data: Dict):
    """Verifies and reports source video counts, sequence counts, class distributions, and active videos BEFORE training."""
    ...
    print(f"\n2. Confirmation of Zero Video Overlap:")
    ...
    active_vids_by_split_cls = defaultdict(lambda: defaultdict(set))
    all_seqs = []
    ...
    for cls_name, seq_list in seq_data.items():
        for s in seq_list:
            ...
            active_vids_by_split_cls[sp][c_name].add(v)
            all_seqs.append(s)

    tot_seqs = len(all_seqs)
    print(f"\n3. Sequences per Class in Each Split (Total: {tot_seqs} sequences):")
    print(f"   {'TEMPO Target Class':<32} {'Train':>10} {'Val':>10} {'Test':>10} {'Total':>10} {'Class Share':>12}")
    print("   " + "-" * 88)
    for c in CLASSES:
        ...
        tot_c = tr + va + te
        pct_c = (tot_c / tot_seqs) * 100
        print(f"   {c:<32} {tr:>10d} {va:>10d} {te:>10d} {tot_c:>10d} {pct_c:>11.1f}%")
    print("   " + "-" * 88)
    ...
    print(f"   {'TOTAL SEQUENCES':<32} {tr_tot:>10d} {va_tot:>10d} {te_tot:>10d} {tot_seqs:>10d} 100.0%")

    print(f"\n4. Active Videos per Class in Each Split:")
    print(f"   {'TEMPO Target Class':<32} {'Train (vids)':>12} {'Val (vids)':>12} {'Test (vids)':>12} {'Total Active'}")
    print("   " + "-" * 88)
    for c in CLASSES:
        tr_v = len(active_vids_by_split_cls["train"][c])
        va_v = len(active_vids_by_split_cls["val"][c])
        te_v = len(active_vids_by_split_cls["test"][c])
        tot_v = len(set(active_vids_by_split_cls["train"][c]) | set(active_vids_by_split_cls["val"][c]) | set(active_vids_by_split_cls["test"][c]))
        print(f"   {c:<32} {tr_v:>12d} {va_v:>12d} {te_v:>12d} {tot_v:>12d}")

    print(f"\n5. Untouched Test Split Confirmation:")
    print(f"   ✓ Test split consists of {len(test_vids)} source videos ({te_tot} sequences).")
    print(f"   ✓ Test split is 100% frozen and completely UNTOUCHED until model selection is finished.")
    print("=" * 85 + "\n", flush=True)
...
def fine_tune_spatial_backbone(train_crops: List[Dict], device: torch.device) -> nn.Module:
    ...
    print(f"\n[Phase 1] Fine-tuning ResNet-18 Spatial Backbone on Classroom Crops...", flush=True)

    # Class-balanced sampling across train crops
    ...
    print(f"  Successfully loaded {len(tensors):,} tensors in memory.", flush=True)
    ...
    # Freeze layers 0-6, fine-tune layer4 + fc
    ...
    # Class-weighted loss for spatial backbone to penalize Writing vs Reading errors
    ...
    # Differential learning rates: smaller LR for layer4 backbone, higher LR for fc linear head
...
def train_and_validate_temporal(
    ...
    """Trains a temporal model and evaluates on the 14 validation videos, saving experimental checkpoints separately."""
    print(f"\n  >>> Training ResNet-18 + {arch} on 65-Video Train Split...", flush=True)

    # Class-balanced sequence sampling
    by_class = defaultdict(list)
    for s in train_seqs:
        lbl = s.get("label") or s.get("target_class") or s.get("class_name")
    ...
    for ep in range(TEMPORAL_EPOCHS):
        ...
        if val_m["macro_f1"] > best_val_f1:
            ...
            # Save separate experimental checkpoint
        ...
        if (ep + 1) % 5 == 0 or ep == TEMPORAL_EPOCHS - 1:
            print(f"    [{arch:>4}] Epoch {ep+1:>2}: train_loss={total_loss/max(1, n_samples):.4f} "
                  f"val_acc={val_m['accuracy']:.4f} val_f1={val_m['macro_f1']:.4f} (best: ep {best_ep+1}, f1={best_val_f1:.4f})", flush=True)

        if patience >= TEMPORAL_PATIENCE:
            print(f"    [{arch:>4}] Early stopping at epoch {ep+1}. Best epoch: {best_ep+1} (val_f1={best_val_f1:.4f})", flush=True)
            break
...
def evaluate_on_unseen_test(
    ...
    """Evaluates champion model strictly on the untouched 14-video test set."""
    print(f"\n[Phase 4] Evaluating Champion Model ({arch}) on Untouched 14-Video Test Split...", flush=True)
    model = TemporalSequenceClassifier(
        temporal_type=arch,
        hidden_dim=TEMPORAL_HIDDEN_DIM,
        num_layers=TEMPORAL_NUM_LAYERS,
    ...
    test_ds = SequenceFeatureDataset(test_seqs, feature_dict, augment=False)
    ...
    # Per-video breakdown for each of the 14 unseen videos
    ...
    for vid, rec in sorted(video_records.items()):
        ...
        per_video_metrics[vid] = {
            ...
            "class_distribution": dict(Counter(CLASSES[i] for i in v_yt)),
        }

    # Prediction class distribution
    pred_distribution = dict(Counter(CLASSES[i] for i in y_pred))

    # Writing -> Reading confusion metric
    w_idx = CL2I["Writing"]
    r_idx = CL2I["Reading"]
    writing_total = int((y_true == w_idx).sum())
    writing_correct = int(((y_true == w_idx) & (y_pred == w_idx)).sum())
    writing_as_reading = int(((y_true == w_idx) & (y_pred == r_idx)).sum())

    writing_reading_stats = {
        "writing_total": writing_total,
        "writing_correct": writing_correct,
        "writing_as_reading": writing_as_reading,
        "misclass_rate": round(writing_as_reading / max(1, writing_total), 4),
    }
    ...
    return {
        ...
        "writing_reading_stats": writing_reading_stats,
    }
...
def main():
    ...
    print("=" * 80, flush=True)
    print("  TEMPO — Targeted Model/Data Improvement Experiment Benchmark", flush=True)
    print("=" * 80, flush=True)
    ...
    print("\n[Data] Loading verified dataset index and 16-frame sequences...", flush=True)
    rows = []
    ...
    # 3. Fine-tune ResNet-18 Spatial Backbone with Targeted Enhancements
    ...
    # 5. Train and Compare RNN, LSTM, and GRU on Validation Split
    ...
    # 6. Validation Comparison Table
    print("\n" + "=" * 85, flush=True)
    print("  TEMPORAL ARCHITECTURE COMPARISON (14-Video Validation Split)", flush=True)
    print("=" * 85, flush=True)
    print(f"  {'Architecture':<18} {'Accuracy':>10} {'Macro-F1':>10} {'Macro-Prec':>12} {'Macro-Rec':>10} {'Time':>8}", flush=True)
    print("  " + "-" * 78, flush=True)
    ...
    for arch, r in bench_results.items():
        ...
        print(f"  ResNet-18 + {arch:<8} {m['accuracy']:>10.4f} {m['macro_f1']:>10.4f} "
              f"{m['macro_precision']:>12.4f} {m['macro_recall']:>10.4f} {r['duration_sec']:>7.1f}s{star}", flush=True)
    print("=" * 85, flush=True)

    champion = bench_results[best_arch]
    print(f"\nChampion Model Selected by Validation Macro-F1: ResNet-18 + {best_arch} (F1: {champion['val_metrics']['macro_f1']:.4f})", flush=True)

    # 7. Evaluate Selected Champion Model ONCE on the Untouched 14-Video Test Split
    test_eval = evaluate_on_unseen_test(
        best_arch,
        champion["state_dict"],
        test_seqs,
        feature_dict,
    ...
    test_m = test_eval["overall"]
    print_metrics_table(f"FINAL EVALUATION: ResNet-18 + {best_arch} on Completely Unseen 14-Video Test Split", test_m)

    # 8. Per-Video Breakdown for the 14 Unseen Videos
    print("\n" + "=" * 85, flush=True)
    print("  PER-VIDEO METRICS (Across All 14 Completely Unseen Test Videos)", flush=True)
    print("=" * 85, flush=True)
    print(f"  {'Video ID':<22} {'Sequences':>10} {'Accuracy':>10} {'Macro-F1':>10} {'Top Classes Present'}", flush=True)
    print("  " + "-" * 82, flush=True)
    for vid, vm in test_eval["per_video"].items():
        top_cls = ", ".join(f"{k[:4]}:{v}" for k, v in sorted(vm["class_distribution"].items(), key=lambda x: -x[1])[:3])
        print(f"  {vid:<22} {vm['sequences']:>10d} {vm['accuracy']:>10.4f} {vm['macro_f1']:>10.4f}  {top_cls}", flush=True)
    print("=" * 85, flush=True)

    # 9. Prediction Class Distribution on Test Set
    print("\n  Prediction Class Distribution on Test Set:", flush=True)
    for c in CLASSES:
        cnt = test_eval["pred_distribution"].get(c, 0)
        pct = (cnt / max(1, sum(test_eval["pred_distribution"].values()))) * 100
        print(f"    {c:<32}: {cnt:>5d} ({pct:>5.1f}%)", flush=True)

    # 10. Specific Writing -> Reading Confusion Breakdown
    wr_stats = test_eval["writing_reading_stats"]
    print("\n" + "=" * 80, flush=True)
    print("  TARGETED FAILURE PATTERN ANALYSIS: WRITING -> READING CONFUSION", flush=True)
    print("=" * 80, flush=True)
    print(f"  Total Ground Truth Writing Sequences in Test Set : {wr_stats['writing_total']}", flush=True)
    print(f"  Correctly Classified as Writing                 : {wr_stats['writing_correct']} ({wr_stats['writing_correct']/max(1, wr_stats['writing_total'])*100:.1f}%)", flush=True)
    print(f"  Misclassified as Reading                        : {wr_stats['writing_as_reading']} ({wr_stats['misclass_rate']*100:.1f}%)", flush=True)
    print("=" * 80, flush=True)

    # 11. Acceptance Gate Evaluation
    print("\n" + "=" * 80, flush=True)
    print("  ACCEPTANCE GATE EVALUATION (Target: Accuracy > 85% AND Macro-F1 > 85%)", flush=True)
    print("=" * 80, flush=True)
    print(f"  Unseen-Video Test Accuracy : {test_m['accuracy'] * 100:.2f}%  (Gate: > 85.00%)", flush=True)
    print(f"  Unseen-Video Test Macro-F1 : {test_m['macro_f1'] * 100:.2f}%  (Gate: > 85.00%)", flush=True)

    passed_gate = test_m["accuracy"] > 0.85 and test_m["macro_f1"] > 0.85

    if passed_gate:
        print("\n  >>> ACCEPTANCE GATE PASSED! <<<", flush=True)
        print("  Updating production checkpoint models/classroom_temporal_model.pth...", flush=True)

        from app.ml.model import ResNet18TemporalModel
        prod_model = ResNet18TemporalModel(
            temporal_type=best_arch,
            hidden_dim=TEMPORAL_HIDDEN_DIM,
            num_layers=TEMPORAL_NUM_LAYERS,
            num_classes=NUM_CLASSES,
        ...
        metadata = {
            ...
            "metrics": {
                ...
                "weighted_f1": test_m["accuracy"],
            },
            "all_model_benchmarks": {
                arch: {
                    "val_accuracy": bench_results[arch]["val_metrics"]["accuracy"],
                    "val_macro_f1": bench_results[arch]["val_metrics"]["macro_f1"],
                    "duration_sec": bench_results[arch]["duration_sec"],
                }
                for arch in bench_results
            },
            "training_data": {
                "source": "Smart Classroom Behavior Dataset (93 videos, Whiffe SCB5 + CCNUZFW)",
                "split": "70/15/15 video-level strict separation (0 video leakage)",
                "train_videos": 65,
                "val_videos": 14,
                "test_videos": 14,
                ...
                "crops_margin": "+20% width, +10% top, +35% bottom for desk/hand context",
                "backbone_finetuned": True,
            },
        }
        ...
        print("\n  >>> ACCEPTANCE GATE NOT MET: Accuracy <= 85% or Macro-F1 <= 85% <<<", flush=True)
        print("  Safety rule enforced: Production checkpoint models/classroom_temporal_model.pth is UNCHANGED.", flush=True)
        print("  Safety rule enforced: Production metadata models/model_metadata.json is UNCHANGED.", flush=True)
        print("  Experimental checkpoints saved separately to models/checkpoints/.", flush=True)
